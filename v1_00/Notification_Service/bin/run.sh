...
#
docker run -p $portToUse:$portToUse --name stage2_notification$portToUse \
    --net=isolated_nw \
    -e portToUse=$portToUse \
    -e serverName="$serverName" \
    -e TZ=`date +%Z` \
    -v $PWD/datavolume:/Notification_Service/datavolume \
    -d dsanders/stage2_notification /bin/bash -c /Notification_Service/startup.sh \
