"""
    ...
    Date:        26 Jan 2016
    ------------------------------------------------------------------------
    Overivew:    The Notification_Service application models a Notification_Service handsfree 
    ...
    App Structure
    -------------
    /Notification_Service                     - Root directory.
       runserver.py                - this file.
       startup.sh                  - the startup command when containerized.
     * Notification_Service_uwsgi.ini         - UWSGI ini file - only in container.
     * Notification_Service_uwsgi.conf        - UWSGI config file - only in container.
     - /bin                        - start, stop, & build - NOT in container.
       /Notification_Service                  - Package root.
       ...
       /Notification_Service_Boundary         - The boundary interfaces.
...
# Import the app, and api modules from the Notification_Service app (__init__.py) so that
