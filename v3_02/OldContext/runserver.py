"""
    ...
    Date:        26 Jan 2016
    ------------------------------------------------------------------------
    Overivew:    The Context application models a Context handsfree 
     ...
     * Context_uwsgi.ini         - UWSGI ini file - only in container.
     * Context_uwsgi.conf        - UWSGI config file - only in container.
     - /bin                        - start, stop, & build - NOT in container.
       ...
       /Context_Boundary         - The boundary interfaces.
...
# Import the app, and api modules from the Context app (__init__.py) so that
