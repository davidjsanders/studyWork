...
class Notification_DB(object):
    database_name = 'datavol/bluetooth.db'
    ...
    def query_one(self, key):
        ...
        db_cursor.execute(
            'select key, value from bluetooth where key = ?',
            (key,) \
        )
    ...
    def query_all(self):
        ...
        try:
            ...
            db_cursor.execute(
                'select key, value from bluetooth'
            )
    ...
    def delete_one(self, key):
        try:
            ...
            db_cursor = db_connection.cursor()
            db_cursor.execute(
                'delete from bluetooth where key = ?', (key,)
            )
    ...
    def insert(self, note):
        ...
        try:
            ...
            db_cursor.execute('select ifnull(max(key),0) from bluetooth')
            db_records = db_cursor.fetchall()
            note.identifier = db_records[0][0] + 1

            db_cursor.execute( \
                'insert into bluetooth (key, value) '+ \
                'values (?, ?)', \
