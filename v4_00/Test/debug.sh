...
while getopts "s:hp:n:" param; do
    case "$param" in
        s) save_param="-v "$OPTARG"/datavolume:/Test/datavolume"
           ;;
        p) port=$OPTARG
...
echo "Running contianer: test_$container_name "
docker run \
    -p $port:$port \
    --name "test_$container_name" \
    ${port_param} \
    ${save_param} \
    ${server_name} \
    -it dsanderscan/mscit_v1_00_test
echo -n "Stopping contianer: test_$container_name = "
docker stop "test_$container_name"
echo -n "Removing contianer: test_$container_name = "
docker rm "test_$container_name"
