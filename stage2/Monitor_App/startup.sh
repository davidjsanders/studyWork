...
#
echo "server {" > Monitor_App_uwsgi.conf
echo "    listen      ${portToUse};" >> Monitor_App_uwsgi.conf
echo "    server_name ${serverName};" >> Monitor_App_uwsgi.conf
echo "    charset     utf-8;" >> Monitor_App_uwsgi.conf
echo "    client_max_body_size 75M;" >> Monitor_App_uwsgi.conf
echo "" >> Monitor_App_uwsgi.conf
echo "    location / { try_files \$uri @yourapplication; }" >> Monitor_App_uwsgi.conf
echo "    location @yourapplication {" >> Monitor_App_uwsgi.conf
echo "        include uwsgi_params;" >> Monitor_App_uwsgi.conf
echo "        uwsgi_pass 127.0.0.1:3031;" >> Monitor_App_uwsgi.conf
echo "    }" >> Monitor_App_uwsgi.conf
echo "}" >> Monitor_App_uwsgi.conf
#
# Build the notifications_uqsgi.ini file
#
echo "[uwsgi]" > Monitor_App_uwsgi.ini
echo "socket=127.0.0.1:3031" >> Monitor_App_uwsgi.ini
echo "chdir = /Monitor_App/" >> Monitor_App_uwsgi.ini
echo "wsgi-file=runserver.py" >> Monitor_App_uwsgi.ini
echo "callable=app" >> Monitor_App_uwsgi.ini
echo "workers=2" >> Monitor_App_uwsgi.ini
echo "processes=4" >> Monitor_App_uwsgi.ini
echo "threads=1" >> Monitor_App_uwsgi.ini
echo "stats=127.0.0.1:9191" >> Monitor_App_uwsgi.ini
echo "stats-http=127.0.0.1:9191" >> Monitor_App_uwsgi.ini
#
...
ln -s /Monitor_App/Monitor_App_uwsgi.conf /etc/nginx/conf.d/Monitor_App_uwsgi.conf
service nginx start
#
...
/flask/bin/uwsgi Monitor_App_uwsgi.ini
