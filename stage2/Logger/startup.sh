...
#
echo "server {" > Logger_uwsgi.conf
echo "    listen      ${portToUse};" >> Logger_uwsgi.conf
echo "    server_name ${serverName};" >> Logger_uwsgi.conf
echo "    charset     utf-8;" >> Logger_uwsgi.conf
echo "    client_max_body_size 75M;" >> Logger_uwsgi.conf
echo "" >> Logger_uwsgi.conf
echo "    location / { try_files \$uri @yourapplication; }" >> Logger_uwsgi.conf
echo "    location @yourapplication {" >> Logger_uwsgi.conf
echo "        include uwsgi_params;" >> Logger_uwsgi.conf
echo "        uwsgi_pass 127.0.0.1:3031;" >> Logger_uwsgi.conf
echo "    }" >> Logger_uwsgi.conf
echo "}" >> Logger_uwsgi.conf
#
# Build the notifications_uqsgi.ini file
#
echo "[uwsgi]" > Logger_uwsgi.ini
echo "socket=127.0.0.1:3031" >> Logger_uwsgi.ini
echo "chdir = /Logger/" >> Logger_uwsgi.ini
echo "wsgi-file=runserver.py" >> Logger_uwsgi.ini
echo "callable=app" >> Logger_uwsgi.ini
echo "workers=2" >> Logger_uwsgi.ini
echo "processes=4" >> Logger_uwsgi.ini
echo "threads=1" >> Logger_uwsgi.ini
echo "stats=127.0.0.1:9191" >> Logger_uwsgi.ini
echo "stats-http=127.0.0.1:9191" >> Logger_uwsgi.ini
#
...
ln -s /Logger/Logger_uwsgi.conf /etc/nginx/conf.d/Logger_uwsgi.conf
service nginx start
#
...
/flask/bin/uwsgi Logger_uwsgi.ini
