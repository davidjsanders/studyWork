"""
    ...
    Date:        26 Jan 2016
    ------------------------------------------------------------------------
    Overivew:    The Bluetooth application models a Bluetooth handsfree 
    ...
    App Structure
    -------------
    /Bluetooth                     - Root directory.
       runserver.py                - this file.
       startup.sh                  - the startup command when containerized.
     * bluetooth_uwsgi.ini         - UWSGI ini file - only in container.
     * bluetooth_uwsgi.conf        - UWSGI config file - only in container.
     - /bin                        - start, stop, & build - NOT in container.
       /Bluetooth                  - Package root.
       ...
       /Bluetooth_Boundary         - The boundary interfaces.
...
# Import the app, and api modules from the Bluetooth app (__init__.py) so that
