...
def printEachTask(taskURI='http://localhost:5000/'):
    data = get_data(taskURI)
    for t in data['tasks']:
        print(getTaskString(t['uri']))

def getTaskString(uri):
    task = getTask(uri)
    returnValue = str(task['id']) + ' ' + task['title'] + ' (' + task['description']+ ') ' + str(task['done'])
    return returnValue
...
def getTask(uri):
    ...
    task = r2.json()['task']
    task0 = task[0]
    return {'id':task0['id'], 'title':task0['title'], 'description':task0['description'], 'done':task0['done']}
